...
def eval_acc(args, model, tokenizer, file_type='test'):
    ...
    #         saved_meta = load_data['saved_meta']
    # else:
    ...
    #                 for target, hidden_state in zip(targets[batch_size], shifted_hidden_states[batch_size]):
    ...
    for step, batch in tqdm(enumerate(eval_dataloader)):
        ...
        with torch.no_grad():
            ...
            #     p_knn = knn(cur_hidden_state, proj_meta[b], saved_hidden_states, saved_target_ids, saved_meta,
            #                 vocab_size)
            #     knn_scores[b] = p_knn
